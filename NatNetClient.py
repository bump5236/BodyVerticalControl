...
class NatNetClient:
    def __init__(self, serverIPAddress="127.0.0.1", localIPAddress="127.0.0.1", multicastAddress="239.255.42.99", commandPort=1510, dataPort=1511):
    ...
    def __dataThreadFunction( self, socket ):
        while True:
            # Block for input
            data, addr = socket.recvfrom( 32768, MSG_DONTWAIT ) # 32k byte buffer size
            if( len( data ) > 0 ):
